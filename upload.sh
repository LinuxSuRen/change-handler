...
target_file=target/change-handler.hpi
...
curl -u $JENKINS_USER:$JENKINS_TOKEN $JENKINS_URL/pluginManager/uploadPlugin -F "name=@$(dirname "${BASH_SOURCE[0]}")/$target_file" --header "Jenkins-Crumb: $issuer"
